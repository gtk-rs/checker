    ...
    while current_info["pos"] < len(content):
        ...
        if clean.startswith("// checker-"):
            if clean == "// checker-ignore-item":
                ...
                print(f'[{path}:{current_info["pos"]}] Found unknown `checker` \
                    command: `{clean[3:]}`')
                errors += 1
                break
            ...
            if content[current_info["pos"]] == "}":
                ...
                if trait_name == "From" and ty_name.startswith("ffi::"):
                    ...
                    if tmp is not None:
                        ...
                        start = kind[impl_for]
                        add_doc_alias_if_needed(
                            content, start, alias, traits, enums, structs, current_info)
            ...
            # directly and not on their implementation.
                ...
                if clean.endswith(");") and "ffi::" in clean:
                    ...
                    add_doc_alias_if_needed(
                        content, current_info["pos"], alias, traits, enums, structs, current_info)
                else:
        ...
        if is_in_trait is not None:
            ...
            if trait_method_pos is None:
                print(
                    f"Cannot find `{fn_name}` in trait `{is_in_trait}`, putting doc aliases on \
                      implementation")
            else:
